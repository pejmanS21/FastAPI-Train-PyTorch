...
def fit(n_epochs, model, train_dataloader, ds, valid_dataloader, val_ds, loss_func, device, optimizer):
    ...
    for epoch in range(n_epochs):
        ...
        loss, acc = train(epoch, n_epochs, model, train_dataloader, loss_func, device, optimizer, len_ds=len(ds))

        torch.cuda.empty_cache()
        val_loss, val_acc = evaluate(model, valid_dataloader, loss_func, device, len_val_ds=len(val_ds))
...
if __name__ == "__main__":
    ...
    fit(1, model, train_dl, dataset, val_dl, val_dataset, loss_func, device=accelerator, optimizer=optimizer)
